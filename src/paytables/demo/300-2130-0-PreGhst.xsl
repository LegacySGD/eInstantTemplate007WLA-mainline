			...
			<lxslt:component prefix="my-ext" functions="formatJson,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						r.push('</td>');
						r.push('<td>');
						r.push(getTranslationByName("turnValue", translations));
						r.push('</td>');
						r.push('<td>');
						r.push(getTranslationByName("cumulativeTotal7s", translations));
						r.push('</td>');
						r.push('<td>');
						...
						for(var i = 0; i < turnDataArray.length; ++i)
						{
							...
							r.push(getTranslationByName("turn", translations)+" "+(i+1).toString());
							r.push('</td>');

							r.push('<td class="tablebody">');
							r.push(turnDataArray[i]);
							r.push('</td>');

							if (turnDataArray[i] === '7')
							{
								numSevens++;
							}

							r.push('<td class="tablebody">');
							r.push(numSevens.toString());
							r.push('</td>');

							r.push('<td class="tablebody">');
							if (turnDataArray[i] === 'X')
							{
								var prizeIndex = getPrizeNameIndex('m11,m10,m9,m8,m7,m6,m5,m4,m3,IW', 'IW');
								r.push(convertedPrizeValues[prizeIndex]);
							}
							else if (i === turnDataArray.length-1)
							{
								var prizeIndex = getPrizeNameIndex('m11,m10,m9,m8,m7,m6,m5,m4,m3,IW', 'm'+numSevens.toString());
								r.push(convertedPrizeValues[prizeIndex]);
							}
							r.push('</td>');
					...
					{
						var turnDataArray = scenario.split(",");
